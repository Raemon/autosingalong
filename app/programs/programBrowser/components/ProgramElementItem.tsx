...
const ProgramElementItem = ({id, version, allVersions, onRemove, onChangeVersion, onClick, onCreateNewVersion, canEdit, isEditing, selectedVersionId, isPendingDeletion}: ProgramElementItemProps) => {
  ...
  return (
    <div className={`text-sm ${isEditing ? 'pl-[8px]' : ''} py-1 flex items-center gap-4 hover:bg-black cursor-pointer group  ${selectedVersionId === id && 'text-primary'} ${isPendingDeletion ? 'opacity-50 line-through' : ''}`} onClick={() => onClick?.(id)}>
